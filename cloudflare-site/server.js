...
// Intelligent Science AI Tutor response fallback for demo mode
function generateScienceAnswer(prompt) {
  const p = prompt.toLowerCase();
  if (p.includes('newton') || p.includes('motion') || p.includes('force')) {
...
app.post('/api/gemini', async (req, res) => {
  try {
    ...
    if (process.env.GEMINI_API_KEY && process.env.GEMINI_API_URL) {
      try {
        const payload = {
          contents: [{ parts: [{ text: `You are an expert CBSE Science tutor for Class 6 to 10. Answer concisely: ${prompt}` }] }]
        };
    ...
    // Smart science answer fallback
    const answer = generateScienceAnswer(prompt);
    res.json({ answer, model: 'A2Z Science AI Engine v2' });
  } catch (err) {
