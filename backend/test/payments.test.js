...
test('Razorpay webhook signature verifies the raw payload', () => {
  ...
  const cryptoSignature=await import('node:crypto').then(({default:c})=>c.createHmac('sha256',secret).update(raw).digest('hex'));
  assert.equal(verifyWebhookSignature(raw,cryptoSignature,secret),true);
  assert.equal(verifyWebhookSignature(raw+' ',cryptoSignature,secret),false);
});
